document.addEventListener('DOMContentLoaded', function() {
    // Finds all main menu headers (h3) in the user sidebar
    const menuHeaders = document.querySelectorAll('.sidebar-menu h3');
    ...
    menuHeaders.forEach(header => {
        ...
        link.addEventListener('click', function(e) {
            ...
            // Check if the link has sub-links (to avoid collapsing simple links)
            const nextSibling = header.nextElementSibling;
            if (nextSibling && nextSibling.classList.contains('sub-link')) {
                e.preventDefault(); // Prevent default link navigation for main headers with sub-links
